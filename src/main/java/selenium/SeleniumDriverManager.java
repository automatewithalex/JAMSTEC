...
public class SeleniumDriverManager {
    ...
    public void createDriver(BrowserTypes browserType, Dimension screenSize) {
        switch (browserType) {
            case CHROME -> createChromeDriver(screenSize);
            case CHROME_H -> createChromeHeadlessDriver(screenSize);
            case FIREFOX -> createFirefoxDriver(screenSize);
            default -> throw new IllegalArgumentException("Unsupported browser type: " + browserType + ". Supported browsers are: " + Arrays.toString(BrowserTypes.values()));
        }
    }
